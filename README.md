# Shivam-Singh
<h1 align="center">Hi 👋, I'm Shivam Singh</h1>

<h3 align="center">
Java Backend Developer | Spring Boot | Microservices | AWS
</h3>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?lines=Java+Backend+Developer;Spring+Boot+Expert;Microservices+Architecture;AWS+Cloud+Engineer&center=true&width=500&height=50">
</p>

---

## 🚀 About Me
- 💻 2+ years of experience in **Java Backend Development**
- ⚙️ Skilled in **Spring Boot, Microservices, REST APIs**
- ☁️ Hands-on with **AWS, Docker, Kubernetes, CI/CD**
- 📈 Focused on **scalable & high-performance systems**

---

## 🛠️ Tech Stack

<p align="center">
<img src="https://skillicons.dev/icons?i=java,spring boot,restapi,aws,docker,kubernetes,jenkins,react,js,mysql,html,css" />
</p>

---

## 📌 Featured Project

### 🔹 CapRushing (Cloud-Native Application)
- Built backend using **Spring Boot & Microservices**
- Deployed on **AWS with Kubernetes**
- Automated pipelines using **Jenkins (CI/CD)**
- Designed scalable REST APIs

---

## 📊 GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight" />
</p>

---

## 🔥 Streak Stats

<p align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=tokyonight" />
</p>

---

## 🔗 Connect with Me

<p align="center">
<a href="https://linkedin.com/in/YOUR_LINK"><img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin"></a>
<a href="https://github.com/YOUR_USERNAME"><img src="https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github"></a>
</p>

---

<p align="center">
⭐ "Building scalable backend systems with clean architecture"
</p>
